...
http.createServer(function (req, res) {
  ...
  if (iteration >= maxIteration){
    ...
    var mostFreqGraphIndx = 0;
    var highestFreq = recordGraph[0][1];
    console.log('Graph number',recordGraph.length);
    for ( var i =0; i < recordGraph.length; i++){
      //console.log('Graph no '+ i, recordGraph[i][1]);
      if( recordGraph[mostFreqGraphIndx][1] < recordGraph[i][1]){
        mostFreqGraphIndx = i;
        console.log('mostFreqGraphIndx updated to',mostFreqGraphIndx);
        console.log('repetetion time',recordGraph[mostFreqGraphIndx][1])
    ...
    plt.plotReport(dtx,recordGraph[mostFreqGraphIndx][0],recordGraph[mostFreqGraphIndx][1]);
    imgData = '<img src = "'+dcanvas.toDataURL()+'" >';
  ...
  if (iteration >= maxIteration){
    ...
    res.end(
      ...
      '<H3>The most frequent graph with the repetitions in total of '+ iteration +' iterations is as below <H3>' +
      imgData
    )
    process.exit(-1);
  }
